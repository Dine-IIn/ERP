...
export const parseItemsSheet = (csvText: string, existingItems: Item[]): BulkUploadResult<Omit<Item, 'id'>> => {
  ...
  dataRows.forEach((cols, idx) => {
    ...
    const name = cols[1];
    const category = (cols[2] || 'Machined Component') as ItemCategory;
    const drawingNo = cols[3] || '';
    const unit = cols[4] || 'PCS';
    const purchaseUOM = cols[5] || unit;
    const conversionFactor = Number(cols[6]) || 1;
    const inHouseStock = Number(cols[7]) || 0;
    const reorderLevel = Number(cols[8]) || 5;
    const unitPrice = Number(cols[9]) || 0;
    const location = cols[10] || 'Central Store';
    const specification = cols[11] || '';
