...
import { FileText, Search, Plus, Edit, Trash2, X, AlertCircle, Calendar, CheckCircle2, Mail, Download, Layers, MessageSquare, Loader2 } from 'lucide-react';
...
export default function ProformaInvoice({
  ...
  const handleEmail = async (id: string, invoiceNo: string) => {
    ...
    try {
      ...
      let customTitleVal = 'PROFORMA ESTIMATE';
      ...
      if (savedSettings) {
        ...
        customTitleVal = savedSettings.title || 'PROFORMA ESTIMATE';
        ...
        if (defaultTpl) {
          ...
          customTitleVal = defaultTpl.title || 'PROFORMA ESTIMATE';
      ...
      await onEmailInvoice(id, base64, `Proforma_${invoiceNo}.pdf`);
      alert(`Proforma Invoice '${invoiceNo}' successfully emailed with your customized template!`);
    } catch (err: any) {
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            setWhatsappShareData({
                              ...
                              pdfBase64: base64,
                              pdfFilename: `Proforma_${inv.invoiceNo}.pdf`
                            });
                          }}
                        ...
                        <div className="grid grid-cols-2 gap-6 text-[10px] text-slate-700">
                          {pdfCustomizer.showMetadata ? (
                            <div className="space-y-1">
                              ...
                              {pdfCustomizer.showInvoiceDate && <div>Document Date: <span className="font-semibold text-slate-900">{new Date(inv.date).toLocaleDateString()}</span></div>}
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      console.error("Failed to save template settings to proforma:", e);
                    }
                    ...
                    {(inv.items || []).map((it: any) => {
                      ...
                      return (
                        <tr key={it.id || it.productId} className="border-b border-slate-100">
                          ...
                          {pdfCustomizer.colProductCode && (
                            <td style={{ padding: `${pdfCustomizer.tablePadding}px` }} className="font-mono text-slate-655">{prod?.sku || 'N/A'}</td>
                          )}
                          <td style={{ padding: `${pdfCustomizer.tablePadding}px` }} className="text-right font-mono">{it.quantity} {prod?.uom || 'PCS'}</td>
                          {pdfCustomizer.colUnitPrice && (
                            <td style={{ padding: `${pdfCustomizer.tablePadding}px` }} className="text-right font-mono">{currencySymbol}{it.price.toFixed(2)}</td>
          ...
          <div style={{ position: 'absolute', left: '-9999px', top: '-9999px' }}>
            ...
            >
              <div style={{ display: 'flex', flexDirection: 'column', gap: `${pdfCustomizer.sectionSpacing}px` }}>
                ...
                {pdfCustomizer.showBankDetails && bankAccounts.length > 0 && (
                  <div className="border border-slate-200 bg-slate-50 p-3.5 rounded-lg text-slate-700 leading-normal space-y-1.5 mt-8" style={{ fontSize: `${pdfCustomizer.bodyFontSize - 1}px` }}>
                    <span className="text-[8.5px] uppercase font-extrabold text-slate-550 block tracking-wider">Company Remittance Details</span>
                    {bankAccounts.map((ba: any) => (
                      <div key={ba.id} className="grid grid-cols-2 gap-4 border-b border-slate-200/50 pb-1.5 last:border-b-0 last:pb-0">
                        <div>Bank: <span className="font-semibold text-slate-900">{ba.bankName}</span></div>
                        <div>Account Holder: <span className="font-semibold text-slate-900">{ba.accountHolderName}</span></div>
                        <div>Account No: <span className="font-semibold text-slate-900">{ba.accountNumber}</span></div>
                        <div>IFSC Code: <span className="font-semibold text-slate-900">{ba.ifscCode}</span></div>
                      </div>
                    ))}
                  </div>
                )}
