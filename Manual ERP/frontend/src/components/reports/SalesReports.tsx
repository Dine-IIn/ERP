      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="p-5 bg-slate-900/35 border border-slate-800/80 backdrop-blur-xl rounded-2xl space-y-1 flex items-center justify-between">
          <div className="space-y-1">
            <span className="text-slate-500 text-[10px] uppercase font-bold tracking-wider">Total Invoices Issued</span>
            <h3 className="text-2xl font-black text-white font-mono">{salesData.invoiceCount}</h3>
          </div>
        ...
        <div className="p-5 bg-slate-900/35 border border-slate-800/80 backdrop-blur-xl rounded-2xl space-y-1 flex items-center justify-between">
          <div className="space-y-1">
            <span className="text-slate-500 text-[10px] uppercase font-bold tracking-wider">Settled Paid Invoices</span>
            <h3 className="text-2xl font-black text-emerald-450 font-mono">{salesData.paidSalesCount}</h3>
          </div>
        ...
        <div className="p-5 bg-slate-900/35 border border-slate-800/80 backdrop-blur-xl rounded-2xl space-y-1 flex items-center justify-between">
          <div className="space-y-1">
            <span className="text-slate-500 text-[10px] uppercase font-bold tracking-wider">Unpaid / Collections due</span>
            <h3 className="text-2xl font-black text-rose-405 font-mono">{salesData.unpaidSalesCount}</h3>
          </div>
          <Clock className="w-6 h-6 text-rose-405 bg-rose-500/10 p-1 rounded-lg border border-rose-500/10" />
        </div>
      </div>

      {/* Sales Curves and lists */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 p-6 bg-slate-900/35 border border-slate-800/80 backdrop-blur-xl rounded-2xl space-y-4">
          <h3 className="text-sm font-bold text-white flex items-center gap-1.5">
            <TrendingUp className="w-4 h-4 text-indigo-455" />
            Monthly Sales Curve Graphs
          </h3>

          {salesData.monthlySales && salesData.monthlySales.length > 0 ? (
            <div className="space-y-4 pt-2">
              {salesData.monthlySales.map((item, idx) => (
                <div key={idx} className="space-y-1.5">
                  ...
                  <div className="w-full h-3 bg-slate-950 rounded-full overflow-hidden border border-slate-900">
                    <div
                      style={{ width: `${Math.min(100, (item.value / Math.max(...salesData.monthlySales.map(m => m.value || 1))) * 100)}%` }}
            ...
            <div className="p-12 text-center text-slate-550 flex flex-col items-center justify-center">
              ...
              <p className="font-semibold text-xs">No monthly sales data found</p>
            </div>
          )}
        </div>
        ...
        <div className="p-6 bg-slate-900/35 border border-slate-800/80 backdrop-blur-xl rounded-2xl space-y-4">
          <h3 className="text-sm font-bold text-white">Payment Collections efficiency</h3>
          <p className="text-slate-500 text-xs leading-relaxed">
            Corporate overview calculating sales invoice payouts settlement rates.
          </p>
          ...
          <div className="p-5 bg-slate-950/40 border border-slate-850 rounded-xl flex items-center justify-center flex-col text-center space-y-2">
            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Settled ratio</span>
            <h4 className="text-3xl font-black text-indigo-455 font-mono">
              {salesData.invoiceCount > 0 ? Math.round((salesData.paidSalesCount / salesData.invoiceCount) * 100) : 0}%
            </h4>
            <span className="text-[10px] text-slate-400 font-medium">Invoices successfully paid</span>
          </div>
        </div>
      </div>
    </div>
