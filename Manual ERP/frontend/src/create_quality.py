...
file_path = "d:\\ERP\\Manual ERP\\frontend\\src\\components\\QualityMaintenance.tsx"
        ...
        {activeTab === 'inspections' && (
          <div className="flex flex-col gap-6 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {[
                { title: 'Pass Rate (MTD)', val: '94.5%', icon: ShieldCheck, color: 'text-emerald-500', bg: 'bg-emerald-500/10' },
                { title: 'Pending Inspections', val: '18', icon: Clock, color: 'text-indigo-500', bg: 'bg-indigo-500/10' },
                { title: 'Open Defects', val: '7', icon: ShieldAlert, color: 'text-red-500', bg: 'bg-red-500/10' },
                { title: 'Machines Down', val: '1', icon: Wrench, color: 'text-amber-500', bg: 'bg-amber-500/10' }
              ].map((kpi, i) => (
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-[var(--border-color)] bg-[var(--bg-primary)]/50">
                      <th className="p-4 text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">Insp ID</th>
                      <th className="p-4 text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">Item / Batch</th>
                      <th className="p-4 text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">Inspector</th>
                  ...
                  <tbody>
                    {inspections.map((insp, i) => (
                      <tr key={i} className="border-b border-[var(--border-color)] hover:bg-[var(--bg-tertiary)]/30 transition-colors">
                        <td className="p-4">
                          <span className="text-sm font-bold text-indigo-400 font-display">{insp.id}</span>
                        </td>
                        <td className="p-4">
                          <p className="text-sm font-bold text-[var(--text-primary)]">{insp.item}</p>
        ...
        {activeTab !== 'inspections' && (
          <div className="h-full flex flex-col items-center justify-center text-center p-12 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-2xl animate-fade-in">
            <div className="w-20 h-20 bg-[var(--bg-tertiary)] rounded-full flex items-center justify-center mb-6">
              {React.createElement(tabs.find(t => t.id === activeTab)?.icon || ShieldCheck, { className: "w-10 h-10 text-[var(--text-muted)]" })}
            </div>
...
with open(file_path, "w", encoding="utf-8") as f:
    f.write(content)
print("QualityMaintenance created.")
