...
async function createAndEmitNotification(params: {
  ...
  try {
    ...
    if (ioInstance) {
      ioInstance.to(params.userId).emit('notification', notification);
...
export async function listTaxSettings(req: AuthenticatedRequest, res: Response) {
  try {
    const companyId = req.user?.companyId;
...
export async function createTaxSetting(req: AuthenticatedRequest, res: Response) {
  try {
    const companyId = req.user?.companyId;
    const parsed = TaxSettingSchema.safeParse(req.body);
    if (!parsed.success) {
      return res.status(400).json({ error: parsed.error.issues[0].message });
    }
    ...
    const newTax = await prisma.taxSetting.create({
      data: {
        companyId: companyId!,
        ...req.body,
...
export async function updateTaxSetting(req: AuthenticatedRequest, res: Response) {
  try {
    const { id } = req.params;
    const companyId = req.user?.companyId;
    const parsed = TaxSettingSchema.safeParse(req.body);
...
export async function deleteTaxSetting(req: AuthenticatedRequest, res: Response) {
  try {
    const { id } = req.params;
    const companyId = req.user?.companyId;
...
export async function listCurrencies(req: AuthenticatedRequest, res: Response) {
  try {
    const companyId = req.user?.companyId;
...
export async function createCurrency(req: AuthenticatedRequest, res: Response) {
  try {
    const companyId = req.user?.companyId;
...
export async function updateCurrency(req: AuthenticatedRequest, res: Response) {
  try {
    const { id } = req.params;
    const companyId = req.user?.companyId;
    const parsed = CurrencySchema.safeParse(req.body);
...
export async function deleteCurrency(req: AuthenticatedRequest, res: Response) {
  try {
    const { id } = req.params;
    const companyId = req.user?.companyId;
...
export async function updateWorkflow(req: AuthenticatedRequest, res: Response) {
  try {
    const { id } = req.params;
    const companyId = req.user?.companyId;
    const parsed = ApprovalWorkflowSchema.safeParse(req.body);
...
export async function deleteWorkflow(req: AuthenticatedRequest, res: Response) {
  try {
    const { id } = req.params;
    const companyId = req.user?.companyId;
...
export async function listApprovalRequests(req: AuthenticatedRequest, res: Response) {
  try {
    const companyId = req.user?.companyId;
    ...
    const filteredRequests = requests.map(reqItem => {
      ...
      const isMyTurn = reqItem.status === 'PENDING' && activeStep && (
        (activeStep.approverUserId === userId) ||
...
export async function createApprovalRequest(req: AuthenticatedRequest, res: Response) {
  try {
    const companyId = req.user?.companyId;
    const userId = req.user?.userId;
...
export async function submitApprovalAction(req: AuthenticatedRequest, res: Response) {
  try {
    const companyId = req.user?.companyId;
    const userId = req.user?.userId;
    const username = req.user?.username;
...
export async function uploadDocument(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    const fileAbsPath = path.join(UPLOADS_DIR, fileName);
...
export async function addDocumentVersion(req: AuthenticatedRequest, res: Response) {
  try {
...
export async function getBackupLogs(req: AuthenticatedRequest, res: Response) {
  try {
    const companyId = req.user?.companyId;
...
export async function triggerBackup(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    const backupAbsPath = path.join(BACKUPS_DIR, backupName);
    const backupRelativePath = `/backups/${backupName}`;

    // Perform file copy for SQLite backup! Fast & incredibly robust!
    fs.copyFileSync(dbPath, backupAbsPath);
    const stats = fs.statSync(backupAbsPath);
    ...
    if (company && companyId && companyId !== 'superadmin-company') {
      ...
      for (const eb of expiredBackups) {
        const ebPath = path.join(BACKUPS_DIR, eb.backupName);
        try {
...
export async function restoreBackup(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    const backupAbsPath = path.join(BACKUPS_DIR, log.backupName);
    ...
    fs.copyFileSync(backupAbsPath, dbPath);
...
export async function getDashboardLayout(req: AuthenticatedRequest, res: Response) {
  try {
    const companyId = req.user?.companyId;
    const userId = req.user?.userId;
...
export async function saveDashboardLayout(req: AuthenticatedRequest, res: Response) {
  try {
    const companyId = req.user?.companyId;
    const userId = req.user?.userId;
...
export async function toggleUserBackupAccess(req: AuthenticatedRequest, res: Response) {
  try {
    const companyId = req.user?.companyId;
...
export async function testEmailConnection(req: AuthenticatedRequest, res: Response) {
  try {
    const companyId = req.user?.companyId;
