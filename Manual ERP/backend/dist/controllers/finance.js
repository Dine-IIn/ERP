...
exports.setFinanceIo = setFinanceIo;
const express_1 = require("express");
const db_1 = __importDefault(require("../services/db"));
const router = (0, express_1.Router)();
// WebSocket notification helper
let ioInstance = null;
function setFinanceIo(io) {
    ioInstance = io;
}
...
async function logAudit(companyId, userId, username, actionType, moduleName, newValue, oldValue = null) {
    try {
...
router.get('/fiscal-years', async (req, res) => {
    try {
        ...
        const years = await db_1.default.fiscalYear.findMany({
            where: { companyId },
...
router.post('/fiscal-year', async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        ...
        // Check for overlap
        const existing = await db_1.default.fiscalYear.findFirst({
            where: {
                companyId,
                OR: [
        ...
        const newYear = await db_1.default.fiscalYear.create({
            data: {
                companyId: companyId,
                yearName,
                startDate: start,
...
router.post('/fiscal-year/:id/close', async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        const { id } = req.params;
        const fiscalYear = await db_1.default.fiscalYear.findFirst({
            where: { id, companyId }
        });
        if (!fiscalYear) {
        ...
        let retainedEarnings = await db_1.default.chartOfAccounts.findFirst({
            where: { companyId, accountCode: "3000-RE", isDeleted: false }
        });
        if (!retainedEarnings) {
            retainedEarnings = await db_1.default.chartOfAccounts.create({
                data: {
                    companyId: companyId,
                    accountCode: "3000-RE",
        ...
        const accounts = await db_1.default.chartOfAccounts.findMany({
            where: {
                companyId,
        ...
        if (accounts.length > 0) {
            ...
            const entryCount = await db_1.default.journalEntry.count({ where: { companyId } });
        ...
        // Close the year
        const updatedYear = await db_1.default.fiscalYear.update({
            where: { id },
            data: { status: "CLOSED" }
        });
...
router.get('/account/:accountId/ledger', async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        const { accountId } = req.params;
        const account = await db_1.default.chartOfAccounts.findFirst({
            where: { id: accountId, companyId, isDeleted: false }
        });
        if (!account) {
            return res.status(404).json({ error: "GL account not found" });
        }
        const lines = await db_1.default.journalEntryLine.findMany({
            where: {
                accountId,
                journalEntry: {
                    companyId,
...
router.get('/journal-entries', async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        const entries = await db_1.default.journalEntry.findMany({
            where: { companyId },
            include: {
                lines: {
                    include: {
...
router.post('/journal-entry', async (req, res) => {
    try {
        ...
        const activeYear = await db_1.default.fiscalYear.findFirst({
            where: {
                companyId,
                status: "ACTIVE",
        ...
        const count = await db_1.default.journalEntry.count({ where: { companyId } });
        const entryNumber = `JV-${postDate.getFullYear()}-${String(count + 1).padStart(4, '0')}`;
        // 4. Transaction execution
        const newEntry = await db_1.default.$transaction(async (tx) => {
        ...
        if (budgetWarnings.length > 0 && req.user?.userId) {
            for (const warning of budgetWarnings) {
                const dbNotif = await db_1.default.notification.create({
                    data: {
...
router.get('/reports/trial-balance', async (req, res) => {
    try {
        ...
        const trialBalance = await Promise.all(accounts.map(async (acc) => {
            const summary = await db_1.default.journalEntryLine.aggregate({
                where: {
                    accountId: acc.id,
                    journalEntry: {
                        companyId,
...
router.get('/reports/cash-flow', async (req, res) => {
    try {
        ...
        const bankAccounts = await db_1.default.chartOfAccounts.findMany({
            where: {
                companyId,
                accountType: "ASSET",
                OR: [
        ...
        const journalLines = await db_1.default.journalEntryLine.findMany({
            where: {
...
// 5. AP / AR SUBLEDGERS
// ==========================================
router.get('/reports/aging/customer', async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        const customers = await db_1.default.customerMaster.findMany({
            where: { companyId, isDeleted: false }
        });
        const now = new Date();
...
router.get('/reports/aging/vendor', async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        const vendors = await db_1.default.vendorMaster.findMany({
            where: { companyId, isDeleted: false }
        });
        const now = new Date();
...
router.get('/assets', async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        const assets = await db_1.default.assetMaster.findMany({
            where: { companyId },
...
router.post('/asset', async (req, res) => {
    try {
        ...
        const existing = await db_1.default.assetMaster.findFirst({
        ...
        const asset = await db_1.default.assetMaster.create({
            data: {
                companyId: companyId,
                assetCode,
                assetName,
        ...
        await db_1.default.assetDepreciationSchedule.createMany({
            data: schedules
        });
...
router.post('/asset/depreciate', async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        const { scheduleId } = req.body;
        const schedule = await db_1.default.assetDepreciationSchedule.findUnique({
            where: { id: scheduleId },
            include: { asset: true }
        });
        ...
        let depExpenseAcc = await db_1.default.chartOfAccounts.findFirst({
            where: { companyId, accountCode: "5020-DEP", isDeleted: false }
        });
        if (!depExpenseAcc) {
            depExpenseAcc = await db_1.default.chartOfAccounts.create({
                data: {
                    companyId: companyId,
        ...
        let assetGlAcc = await db_1.default.chartOfAccounts.findFirst({
            where: { companyId, accountCode: `1500-${schedule.asset.assetCode}`, isDeleted: false }
        });
        if (!assetGlAcc) {
            assetGlAcc = await db_1.default.chartOfAccounts.create({
                data: {
                    companyId: companyId,
...
router.get('/bank-accounts', async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        const bankAccounts = await db_1.default.bankAccount.findMany({
            where: { companyId },
...
router.post('/bank-account/:bankAccountId/statement', async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        ...
        const statement = await db_1.default.bankStatement.create({
            data: {
                bankAccountId,
        ...
        for (const sLine of statement.lines) {
            ...
            const bankAcc = await db_1.default.bankAccount.findUnique({ where: { id: bankAccountId } });
            if (!bankAcc)
                continue;
            const matchedGlLine = await db_1.default.journalEntryLine.findFirst({
                where: {
            ...
            if (matchedGlLine) {
                await db_1.default.bankStatementLine.update({
                    where: { id: sLine.id },
                    data: {
                        isReconciled: true,
...
router.get('/bank-account/:bankAccountId/unreconciled', async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        const { bankAccountId } = req.params;
        const bankAcc = await db_1.default.bankAccount.findFirst({
            where: { id: bankAccountId, companyId }
        });
        if (!bankAcc) {
            return res.status(404).json({ error: "Bank account not linked" });
        }
        const bankLines = await db_1.default.bankStatementLine.findMany({
            where: {
        ...
        const glLines = await db_1.default.journalEntryLine.findMany({
            where: {
                accountId: bankAcc.glAccountId,
                journalEntry: {
                    companyId,
        ...
        const reconciledGlEntryIds = (await db_1.default.bankStatementLine.findMany({
            where: {
                bankStatement: { bankAccountId },
                isReconciled: true,
...
router.post('/bank-reconcile', async (req, res) => {
    try {
        ...
        const line = await db_1.default.bankStatementLine.findUnique({
        ...
        const journal = await db_1.default.journalEntry.findFirst({
        ...
        const remainingUnreconciled = await db_1.default.bankStatementLine.count({
            where: { bankStatementId: line.bankStatementId, isReconciled: false }
        });
        if (remainingUnreconciled === 0) {
            await db_1.default.bankStatement.update({
